...
public class DemoSimulatorOriginal {
    ...
    private static int[] runSim(int runs, int seed) {
        int[] score = new int[runs];
        int i = 0;
        while(i < runs) {
            // Set the seed for reproducible behaviour
            Random r = seed <= 0 ? new Random() : new Random(seed); // TODO: Change the seed to nothing after finished building
